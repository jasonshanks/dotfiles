...
const vec4 TRAIL_COLOR = vec4(0.65098, 0.85882, 1.0, 1.0);
const float DURATION = 0.3; // IN SECONDS
const float DISTANCE_THRESHOLD = 3.5; // ADDED: Maximum distance for trail effect
...
void mainImage(out vec4 fragColor, in vec2 fragCoord)
{
    ...
    // ADDED: Calculate distance between cursor positions
    vec2 centerCC = getRectangleCenter(currentCursor);
    vec2 centerCP = getRectangleCenter(previousCursor);
    float cursorDistance = distance(centerCC, centerCP);
    
    // ADDED: Check if distance exceeds threshold
    ...
    // MODIFIED: Only calculate trail SDF if enabled
    float sdfTrail = trailEnabled ? getSdfParallelogram(vu, v0, v1, v2, v3) : 1e9; // Large value to disable trail
    ...
    // MODIFIED: Only apply trail effect if enabled
    if (trailEnabled) {
